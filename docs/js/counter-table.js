class CounterTableManager {
    ...
    createTable(processedData, notesLoader) {
        ...
        this.tableBody = document.getElementById('counterDataTableBody');

        if (!this.tableBody) {
            console.error('Counter table body element not found');
            return;
        }
        ...
        if (!data || data.length === 0) {
            console.warn('No counter data to display in table');
            return;
        }
        ...
        data.forEach((entry, index) => {
            ...
            // Calculate total count for this entry
            const totalCount = Array.from(entry.counters.values()).reduce((sum, count) => sum + count, 0);
            ...
            if (index > 0) {
                ...
                const prevTotalCount = Array.from(prevEntry.counters.values()).reduce((sum, count) => sum + count, 0);

                if (prevTotalCount > 0) {
                    changePercent = ((totalCount - prevTotalCount) / prevTotalCount) * 100;
